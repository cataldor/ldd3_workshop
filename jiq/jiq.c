...
static void jiq_print_tasklet(struct tasklet_struct *);
 ...
 */
static void jiq_print_tasklet(struct tasklet_struct *tl)
{
	struct clientdata *data = from_tasklet(data, tl, jiq_tasklet);
...
static int jiq_read_tasklet_show(struct seq_file *m, void *v)
{
	...
	wait_event_interruptible(jiq_wait, READ_ONCE(jiq_data.end));
...
static int jiq_init(void)
{
	...
	tasklet_setup(&jiq_data.jiq_tasklet, jiq_print_tasklet);
